...
spp_list <- data.frame(
  ...
  file_name = c("simple_walleye_pollock"), 
...
Set variables. This... requires more descrption. `seq(from = seq_from, to = seq_to, by = seq_by)`
...
start.time <- Sys.time()
sink(file = paste0(dir_out, srvy, "_simple_logfile.txt"), append = FALSE, split = TRUE)  
purrr::map(
  1:nrow(spp_list), 
...
sink()
write.table(x = (Sys.time() - start.time), file = paste0(dir_out, srvy, "_simple_time.txt"))
```

```{r time}
a <- readLines(con = paste0(dir_out, srvy, "_simple_time.txt") )
cat(paste0("Time difference of ", strsplit(x = a[2], split = " ")[[1]][2], " secs"))
```

```{r sink-results}
cat(readLines(con = paste0(dir_out, srvy, "_simple_logfile.txt") ))  
```
...
# Check `your_mesh$mesh$n` to view the number of vertices.Registered S3 methods overwritten by 'htmltools':
#   method               from
...
#   method         from
#   print.paged_df
# Warning: The model may not have converged: non-positive-definite Hessian matrix.Warning: The model may not have converged: non-positive-definite Hessian matrix.Warning: NaNs producedWarning: NaNs producedWarning: NaNs producedWarning: NaNs producedWarning: NaNs producedWarning: NaNs producedWarning: NaNs producedWarning: NaNs produced✔ Non-linear minimizer suggests successful convergence
