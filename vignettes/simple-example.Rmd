...
spp_list <- data.frame(
  ...
  file_name = c("walleye_pollock_test"), 
...
sink(file = paste0(dir_out, srvy, "_simple_", Sys.Date(), "_logfile.txt"), append=FALSE, split=TRUE)  
purrr::map(
  1:nrow(spp_list), 
...
```{r sink-results}
cat(readLines(con = paste0(dir_out, srvy, "_simple_", Sys.Date(), "_logfile.txt") ))  
```
...
```{r}
aaa <- load(file = paste0(dir_out, paste0(srvy, "_simple_0figures/figures.rdata")))
aaa
```
