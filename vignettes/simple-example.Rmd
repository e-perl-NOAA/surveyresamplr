...
dir_final <- paste0(dir_out, "EBS_simple_results/")
...
sink()
write.csv(x = data.frame(time = as.numeric(Sys.time() - start.time), units = units(Sys.time() - start.time)), file = paste0(dir_out, srvy, "_simple_time.csv"))
```

```{r time}
a <- read.csv(file = paste0(dir_out, srvy, "_simple_time.csv") )
cat(paste0("Time difference of ", round(a$time, 2), " ", a$units))
```
