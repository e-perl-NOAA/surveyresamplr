plot_results <- function(srvy, dir_out, dir_final = NULL) {
  ...
  if (!is.null(tables$index)) {
    ...
    plots$log_biomass <- ggplot(tables$index, aes(x = factor(year), y = log_est)) +
      ...
      theme_minimal() + 
      labs(title = "Log Biomass by Year and Effort",
    ...
    plots$log_biomass_se <- ggplot(tables$index, aes(x = factor(year), y = se)) +
      geom_boxplot(aes(fill = factor(effort))) +
      theme_minimal() +
      labs(title = "Standard Error by Year and Effort",
    ...
    plots$biomass <- ggplot(tables$index, aes(x = factor(year), y = est)) +
      geom_boxplot(aes(fill = factor(effort))) +
      theme_minimal() +
      labs(title = "Biomass by Year and Effort",
           x = "Year", y = "Biomass",
           fill = "Effort")
    ...
    plots$timeseries <- ggplot(tables$index, aes(x = year, y = est)) +
      ...
      theme_minimal() +
      labs(title = "Biomass Timeseries by Effort",
           x = "Year", y = "Biomass",
           color = "Effort")
  ...
  # Save all outputs
  save(file = file.path(dir_final, "analysisoutput.rdata"),
       list = c("tables", "plots"))
