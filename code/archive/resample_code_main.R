...
wd<- "Z:/Projects"
# basedir<-paste0(wd,'Resample-survey-data')
# output <- paste0(wd, "Resample-survey-data/Results")
data <-paste0(wd, "/Resample-survey-data/data")
basedir<-paste0(wd, "/Resample-survey-data")
output <- paste0(wd, "/Resample-survey-data/Results/")
#data <-paste0(wd, "/data/")
arrowtooth <- paste0(output, "Arrowtooth_flounder")
bocaccio <- paste0(output, "Bocaccio")
canary <- paste0(output, "Canary_rockfish")
darkblotched <- paste0(output, "Darkblotched_rockfish")
dover <- paste0(output, "Dover_sole")
lingcod_n <- paste0(output, "Lingcod_north")
lingcod_s <- paste0(output, "Lingcod_south")
longnose <- paste0(output, "Longnose_skate")
pop <- paste0(output, "Pacific_ocean_perch")
dogfish <- paste0(output, "Pacific_spiny_dogfish")
petrale <- paste0(output, "Petrale_sole")
rex <- paste0(output, "Rex_sole")
sablefish <- paste0(output, "Sablefish")
shortspine <- paste0(output, "Shortspine_thornyhead")
widow <- paste0(output, "Widow_rockfish")
yellowtail <- paste0(output, "Yellowtail_rockfish")
...
library(arrow)
library(data.table)
# install.packages("ggplot2", "dplyr", "tidyr", "forcats", "purrr", "readr", "tibble", "doParallel", "sdmTMB")
