test_that("wrapper_surveyIndex has expected structure", {
  skip_if_not_installed("sdmTMB")
  skip_on_cran()

  # Check that the function exists
  expect_true(exists("wrapper_surveyIndex"))

  # Check that the function has the expected parameters
  fn_params <- names(formals(wrapper_surveyIndex))
  expected_params <- c("x", "y", "z", "dir_spp", "spp_info", "n_knots")
  expect_equal(fn_params, expected_params)

  # Check default value for n_knots
  expect_equal(formals(wrapper_surveyIndex)$n_knots, 500)
})

test_that("wrapper_surveyIndex creates expected output structure", {
  skip_if_not_installed("sdmTMB")
  skip_on_cran()
