test_that("wrapper_sdmtmb creates expected output structure", {
  ...
  test_wrapper <- function(x, y, z, dir_spp, spp_info, n_knots = 300) {
    # make mesh
  ...
  expect_equal(nrow(result$index), length(unique(test_grid$year)))
  expect_equal(names(result$index), c("year", "est", "se", "log_est"))
