#### resample survey data: Multiple species, one year
####
#######################################################################################################################################
...
allcatch<-PullCatch.fn(YearRange = "2019", SurveyName = "NWFSC.Combo")
#bio<-PullBio.fn(SurveyName = "NWFSC.Combo")

catch<-allcatch[allcatch$Common_name == "sablefish"|allcatch$Common_name =="canary rockfish"|allcatch$Common_name =="Pacific ocean perch",]

table(catch$Common_name)
catch<-unique(catch)

#create a vector of tows for including or excluding. 
tows<- as.data.frame(catch$Trawl_id)
tows<-unique(tows)
...
tows_assigned <- include_or_exclude(tows, seq(0.1,1.0, by = 0.1))
...
#only keep the 1s
alldata_resampled<-lapply(catch_assigned,function(x){x[x$RandomAssignment==1,]})
...
indicies<- lapply(adr_split, function(x){Biomass.fn(dir = getwd(),
                                                            dat = x,
...
#make plots: index CV
ggplot(data=ests_df, aes(x=Proportion, y=cv, color = species)) + geom_point() +
  geom_smooth(method = "lm", formula = y ~ poly(x, degree = 3), se = F, na.rm = T) + facet_wrap(~species) +
  theme_classic() + theme(axis.text=element_text(color = "black", size=12),
                          axis.title = element_text(color="black",size=16)) +
  labs(x="Proportion of tows kept", y = "Index CV")

setwd(output)
ggsave(filename = 'multi_spp_index_CV_demo.tiff',plot = last_plot() , path = output, width = 12, height = 6, device = 'tiff', dpi = 300)

#make plots: biomass ests
ggplot(data=ests_df, aes(x=Proportion, y=Bhat, color = species)) + geom_point() +
  geom_smooth(method = "lm", formula = y ~ poly(x, degree = 3), se = F, na.rm = T) + facet_wrap(~species, scales = "free") +
...
setwd(output)
ggsave(filename = 'multi_spp_biomass_ests_SEM_demo.tiff',plot = last_plot() , path = output, width = 12, height = 6, device = 'tiff', dpi = 300)
